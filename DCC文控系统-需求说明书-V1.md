...
| 文档版本 | V1.5.18 |
| 编写日期 | 2026-07-21 |
| 最近修订 | 2026-07-24（V1.5.18：借阅跨部门临时预览、外发令牌/水印包、外来登记附件、纸质两步回收、附件实装、纸质筛选） |
| 所属公司 | 米格实验室（霸州需求） |
...
| 预览方式 | **首期：转 PDF 预览**（Office/图片等先转 PDF 再在线看） |
...
| 文件级别 | 独立字段 `file_level`：`L1` 一级（宏观）/ `L2` 二级（部门细则）/ `L3` 三级（表单）；一二级网页不可改正文；三级可网页编辑、**免审批**，每次保存递增**轻量修订号 rN**（见 §4.2 / §19.1） |
...
| 下载 | 无直授且不满足上款时，须「下载二次申请」→ 审批通过后下载水印 PDF；全程留痕 |
...
| 策略 | 规则 |
|---|---|
| 预览水印 | **默认开启**；可由文控在全局水印策略中开关（`apply_preview`）；开启时：`姓名+工号+文件编号+时间` 平铺 + 阶段水印；关闭时预览不叠加水印层 |
| 下载水印 | **强制**：仅输出带水印 PDF（不直接给未加水印原件）；文控不可关闭 |
...
| 能力 | 要求 |
|---|---|
| 全文搜索 | 对已转 PDF 成功的正文建立文本索引；支持标题/编号/正文关键词检索 |
...
| 类别 | 要求 |
|---|---|
| 性能（可判定验收） | 台账列表 P95 < 2s（1 万条内）；预览首屏 < 3s（已转 PDF）；**检索 P95 < 2s** |
...
- [x] 转 PDF 预览 + **下载/打印强制水印**（预览水印可由文控开关）+ 受控打印（震慑+溯源）  
- [x] 纸质受控份 + 回收  
- [x] 修订/作废**变更原因强制**  
...
> 图中「站内信」指复用现网消息；「水印 PDF」指下载/打印强制水印（预览受文控策略开关控制，默认开）。
...
| 三级 | L3 | 表单 | **可直接编辑保存** | **免审批**；每次保存递增轻量修订号 `rN` 并留痕（**不**推进正式 `1.0/2.0`，见 §4.2.2）；新建入库与作废 **仍须审批**；需要正式受控升版时走修订审批 |
...
**下一步**：按 **V1.5.18** 固化表结构与权限（含借阅跨部门预览、外发令牌/水印包、外来附件、纸质两步回收、附件实装）；原型 `web/` 已对齐；后端按 T-03-B 事件模型 + §4.5.1 生效任务 + §13.3 迁移方案实施。验收叠加 §14 + §17 + §18 + §19（含 §19.8～§19.9）。
