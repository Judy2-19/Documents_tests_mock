...
<template>
        <div class="page-card">
          ...
          <el-table :data="pageSlice(roleBorrows,'borrows')" size="small" stripe border>
            ...
            <el-table-column label="操作" width="100" fixed="right" align="center" class-name="ops-col" label-class-name="ops-col">
              <template #default="{ row }">
                <div class="ops-cell">
                <button class="link-btn" @click="openPreview(row,'BORROW')">预览</button>
                <button class="link-btn" @click="toast(row.status==='OVERDUE'?'已催还：'+row.borrowNo:'归还登记：'+row.borrowNo)">{{ row.status==='OVERDUE'?'催还':'归还' }}</button>
                </div>
              </template>
            </el-table-column>
          </el-table>
